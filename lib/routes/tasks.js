...
// (and components with withSession() in client App)
  ...
  .put('/:id', ensureAuth(), (req, res, next) => {
    const { title, taskType, location, contactName, streetAddr, contactPhone, contactEmail, description } = req.body;
    Task
      .findOneAndUpdate(
        { _id: req.params.id, author: req.user.sub, date: new Date() }, //, date: new Date() },
        { title, taskType, location, contactName, streetAddr, contactPhone, contactEmail, description }, // date,
        { new: true }
      )
      .then(task => joinUsers([task], 'author'))
      .then(([task]) => res.send(task))
