...
// (and components with withSession() in client App)
  ...
  .post('/', ensureAuth(), (req, res, next) => {
    ...
    //   return date;
    // }
    const { title, contactName, description } = req.body;
    Task
      .create({ title, contactName, date: new Date(), description, author: req.user.sub }) // date: new Date(),
  ...
  .put('/:id', ensureAuth(), (req, res, next) => {
    ...
    Task
      .findOneAndUpdate(
        ...
        { title, contactName, description }, // date,
        { new: true }
      )
      .then(task => joinUsers([task], 'author'))
      .then(([task]) => res.send(task))
